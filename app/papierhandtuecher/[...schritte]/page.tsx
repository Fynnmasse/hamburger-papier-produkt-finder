...
export function generateStaticParams() {
  return generateCategoryStaticParams('papierhandtuecher')
}

export function generateMetadata({ params }: { params: Promise<{ schritte: string[] }> }) {
  return params.then(p => generateCategoryMetadata('papierhandtuecher', p.schritte))
}

export default async function Page({ params }: { params: Promise<{ schritte: string[] }> }) {
  const { schritte } = await params
  return <CategoryPage kategorie="papierhandtuecher" segments={schritte} />
}
