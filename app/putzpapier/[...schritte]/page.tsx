...
export function generateStaticParams() {
  return generateCategoryStaticParams('putzpapier')
}

export function generateMetadata({ params }: { params: Promise<{ schritte: string[] }> }) {
  return params.then(p => generateCategoryMetadata('putzpapier', p.schritte))
}

export default async function Page({ params }: { params: Promise<{ schritte: string[] }> }) {
  const { schritte } = await params
  return <CategoryPage kategorie="putzpapier" segments={schritte} />
}
