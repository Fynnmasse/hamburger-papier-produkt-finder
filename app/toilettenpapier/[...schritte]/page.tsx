...
export function generateStaticParams() {
  return generateCategoryStaticParams('toilettenpapier')
}

export function generateMetadata({ params }: { params: Promise<{ schritte: string[] }> }) {
  return params.then(p => generateCategoryMetadata('toilettenpapier', p.schritte))
}

export default async function Page({ params }: { params: Promise<{ schritte: string[] }> }) {
  const { schritte } = await params
  return <CategoryPage kategorie="toilettenpapier" segments={schritte} />
}
